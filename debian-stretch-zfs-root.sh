...
DEBIAN_FRONTEND=noninteractive chroot /target /usr/bin/apt-get install --yes firmware-linux firmware-iwlwifi firmware-realtek  linux-image-amd64 grub2-common $GRUBPKG zfs-initramfs zfs-dkms
...
ETHDEV=$(ip route list match 0.0.0.0/0 | awk '{print $5; exit}')
